 ...
 * 
 * CRITICAL LEARNINGS:
 * 1. DefaultAzureCredential requires AZURE_CLIENT_ID env var to select correct identity
 * 2. Model/deployment name must be injected server-side, not trusted from frontend
 * 3. Token caching is essential to avoid rate limits on identity endpoint
 */
...
const app = express();
const PORT = process.env.PORT || 80;  // Container Apps expects port 80
...
// CRITICAL: AZURE_CLIENT_ID env var must be set for DefaultAzureCredential to work
const credential = new DefaultAzureCredential();

// Token cache - prevents rate limiting on identity endpoint
...
// Health check endpoint (used by Container App probes)
...
// Frontend uses this to display configuration in UI
 ...
 * Caches token to avoid rate limiting on identity endpoint
 */
 ...
 * CRITICAL: Always override model with server-configured deployment
 */
app.post('/api/openai/responses', async (req, res) => {
  try {
    ...
    // CRITICAL: Override the model field with the backend-configured deployment
    // Frontend may send empty or incorrect model, backend always uses correct one
    const requestBody = {
      ...req.body,
 ...
 * Browser Speech SDK cannot use managed identity directly,
 * so backend provides token that frontend can use
 */
app.get('/api/speech/token', async (req, res) => {
  try {
    ...
    res.json({
      token: token,
      region: AZURE_SPEECH_REGION,
      expiresIn: 600, // Token valid for ~60 min, but tell client 10 min for safety
    });
  } catch (error) {
